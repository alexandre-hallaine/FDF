...
void dda(mlx_image_t *image, t_dot dot[2]) {
    ...
    double step_amount =
            ABS(position_delta[0]) > ABS(position_delta[1]) ? ABS(position_delta[0]) : ABS(position_delta[1]);
    for (double step = 0; step <= step_amount; step++) {
        if (position[0] >= 0 && position[0] < image->width && position[1] >= 0 && position[1] < image->height)
            mlx_put_pixel(image, position[0], position[1],
                          (short) rgb[0] << 24 | (short) rgb[1] << 16 | (short) rgb[2] << 8 | 0xFF);
